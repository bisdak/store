...
def test_uow_can_retrieve_a_batch_and_allocate_to_it(session_factory):
    ...
    insert_batch(session, "batch1", "WORKBENCH", 100, None)
    session.commit()

    uow = unit_of_work.SqlAlchemyUnitOfWork(session_factory)
    with uow:
        product = uow.products.get(sku="WORKBENCH")
        line = model.OrderLine("o1", "WORKBENCH", 10)
        product.allocate(line)
        uow.commit()

    batchref = get_allocated_batch_ref(session, "o1", "WORKBENCH")
...
def test_rolls_back_uncommitted_work_by_default(session_factory):
    uow = unit_of_work.SqlAlchemyUnitOfWork(session_factory)
    with uow:
        insert_batch(uow.session, "batch1", "BACKBOARD", 100, None)
